...
export default function RegistroPage() {
  ...
  return (
    <main className="min-h-[100dvh] bg-[#D6EFFB] flex flex-col pb-20">
      <header className="w-full px-5 py-6 flex items-center gap-4 bg-white/50 backdrop-blur-md sticky top-0 z-20 border-b border-[#A9CFEA]/20">
        <Link
          href="/"
          aria-label="Volver al inicio"
          className="p-2 -ml-2 rounded-full hover:bg-[#1D5FCC]/10 text-[#1D5FCC] transition-colors"
        >
      ...
      <div className="max-w-[600px] w-full mx-auto px-5 py-8 md:py-12 space-y-8">
        <div className="text-center">
          <div className="inline-flex items-center justify-center w-16 h-16 bg-white rounded-2xl shadow-sm border border-[#A9CFEA]/30 mb-5 text-3xl">
            🏪
          </div>
          <h1 className="font-headline font-bold text-[#0E2A52] text-2xl md:text-3xl mb-3">
            {miTienda ? 'Configuración de tu tienda' : 'Crea tu vitrina digital'}
          </h1>
          <p className="text-[#4C6B8F] text-sm md:text-base leading-relaxed max-w-md mx-auto">
            {miTienda 
        ...
        <div className="bg-white rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] p-6 md:p-8 border border-[#A9CFEA]/30">
          {saveError && (
