...
export default function ProductCard({
  ...
  return (
    ...
    >
      <div className="relative aspect-square w-full bg-[#D6EFFB] overflow-hidden group">
        {producto.fotoUrl ? (
          <Image
        ...
        {unavailable && (
          <div className="absolute inset-0 bg-white/40 backdrop-blur-[2px] flex items-center justify-center z-10">
            <span className="bg-[#0E2A52] text-white text-[11px] font-bold px-3 py-1.5 rounded-full uppercase tracking-wider shadow-lg">
              Agotado
            </span>
          </div>
        )}
          ...
          <div className="absolute top-2 left-2 flex gap-1.5 z-20">
            {onEdit && (
              <button
                ...
                className="p-2 rounded-full bg-white/90 backdrop-blur-sm shadow-[0_2px_8px_rgba(0,0,0,0.1)] text-[#0E2A52] hover:bg-white hover:scale-110 transition-all"
              >
            ...
            {onDelete && (
              <button
                ...
                className="p-2 rounded-full bg-white/90 backdrop-blur-sm shadow-[0_2px_8px_rgba(0,0,0,0.1)] text-[#1D5FCC] hover:bg-white hover:scale-110 transition-all"
              >
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
      ...
      <div className="p-3 md:p-4 flex flex-col flex-grow bg-white">
        <h3
          className={`text-[14px] md:text-[15px] font-bold leading-tight mb-1 line-clamp-2 ${
            unavailable ? 'line-through text-[#4C6B8F]/60' : 'text-[#0E2A52]'
          }`}
        >
          {producto.nombre}
        </h3>
        ...
        {producto.descripcion && (
          <p className="text-[12px] text-[#4C6B8F]/80 line-clamp-2 leading-relaxed mb-2 flex-grow">
            {producto.descripcion}
          </p>
        )}
        ...
        <div className="mt-auto pt-1">
          <p
            className={`font-headline font-black text-[15px] md:text-[17px] ${
              unavailable ? 'text-[#4C6B8F]/50' : 'text-[#1D5FCC]'
            }`}
          >
            {formatPrice(producto.precio)}
          </p>
          {tiendaNombre && (
            <p className="text-[11px] font-medium text-[#4C6B8F]/60 truncate mt-0.5">{tiendaNombre}</p>
          )}
        </div>
      </div>
    </article>
